...
public class Board {
  ...
  /**
   * tracks how many turns have elapsede
   */
  private int turn;
  private int virtualTurn;

  /** 
   * list of valid moves; needs to be updated every turn, but the getter does that
   */
  //ArrayList<Move> moveList;

  /** 
   * tracks whether the moveList is up-to-date
   */
  ...
  private void changeTile(int row, int col, Chip c) throws IndexOutOfBoundsException {
    ...
    if ( row < 0 || row >= width || 
         col < 0 || col >= width ) 
    {
      throw new IndexOutOfBoundsException("changeTile row and col (\"" + row + "\", \"" + 
          col + "\") must satisfy: 0 <= index < " + width);
    }
  ...
  /**
   * returns a list of all the legal moves for the current player given the
...
//     }
  ...
  */
  ...
  public void pushMove(Move m) {
    ...
    //System.out.println("Move: " + m.toString() + " was pushed");
  } 
  ...
  public boolean legalMove(Move move) {
    ...
    return getLegalMoves().contains(move);  // ...then see if our move is in it
  }
...
/**
  ...
  private Move popMove() {
    ...
    //System.out.println("popped : " + m.toString());
    return m;
  }
...
public void undoMove(boolean apply) throws InvalidMoveException {
    Chip friend, enemy;
    int row, col;
    int rowChange, colChange;
    Move move; //move to undo

    move = popMove(); //move is the move to undo
    if (move == null) {
        System.err.println("popped move was null");
        System.exit(1);
    }
    //System.out.println("Popping move " + move.toString() + " on turn " + (turn-1));
    this.undoEndTurn();  
    
    /* special case for first 2 turns */
    if (turn == 0) {
        assert (!moveStack.empty());
        changeTile(move.pointFrom().row(), move.pointFrom().col(), Chip.BLACK); //on first turn
        return;
    } 
    if (turn == 1) {
        if (getTile(move.pointFrom().row(), move.pointFrom().col()).getChip() == Chip.NONE) {
            changeTile(move.pointFrom().row(), move.pointFrom().col(), Chip.WHITE);
            return;
        } else {
            throw new InvalidMoveException("ERROR: chip not NONE when undoing turn 1");
        }
    }
//         row = move.pointFrom().row() + rowChange;
//         col = move.pointFrom().col() + colChange;
...
//             if (apply) {
//               assert (!blackToPlay);
//               changeTile(move.pointFrom().row(), move.pointFrom().col(), Chip.WHITE); //on first turn
//               //change the "removed" chip back to white
//               //this.undoEndTurn();
//             }
//             return;
//         }
...
//     }
    ...
    // /* check if chip to move is ours */
//     if (getTile(move.pointTo()).getChip() != friend) {
//       throw new InvalidMoveException("Error: UNDO trying to move chip not owned by current player(" + move.pointFrom().row() + ", " + move.pointFrom().col() + ")");
//     }
// 
//     /* check if place the chip came from is empty */
//     if (getTile(move.pointFrom()).getChip() != Chip.NONE) {
//       throw new InvalidMoveException("Error: UNDO trying to move chip into an occupied square(" + move.pointTo().row() + ", " + move.pointTo().col() + ")");
//     }
    ...
    colChange = 0;
    if (move.pointFrom().row() == move.pointTo().row() &&  // horizontal move
        move.pointFrom().col() > move.pointTo().col())  // to the left, undo to right
    {
      colChange = 1; //instead of -1
    } else if (move.pointFrom().row() == move.pointTo().row() &&  // horizontal
               move.pointFrom().col() < move.pointTo().col())  // to the right
    {
      colChange = -1; //instead of +1
               ...
               move.pointFrom().row() > move.pointTo().row())  // up
    {
      rowChange = 1; //instead of -1
    } else if (move.pointFrom().col() == move.pointTo().col() &&  //vertical
               move.pointFrom().row() < move.pointTo().row())  // down
    {
      rowChange = -1; //instead of 1
    } else {
      throw new InvalidMoveException("Error: UNDO trying to move chip in a non-cardinal direction");
    }

    row = move.pointTo().row(); //move.pointFrom().row();
    col = move.pointTo().col();  //move.pointFrom().col();
    /* check each square between start and end, see if they contain the right things */
    ...
    // for (int i=1, r=row+rowChange, c=col+colChange;  // init indicies
//         //r!=move.pointTo().row() || c!=move.pointTo().col();  // go until we get to the "to" point 
//         r!=move.pointFrom().row() || c!=move.pointFrom().col();  // go until we get to the "from" point
//         i++, r+=rowChange, c+=colChange)  // update indicies
//         {
//           /* check whether this tile has the right thing in it */
//           if (i%2 == 0 && getTile(r, c).getChip() != Chip.NONE) {
//             throw new InvalidMoveException("Error: UNDO trying to jump chip into an occupied square (" + r + ", " + c + ")");
//           } else if (i%2 != 0 && getTile(r, c).getChip() == friend) {
//             throw new InvalidMoveException("Error: UNDO trying to jump chip over a friendly piece (" + r + ", " + c + ")");
//           } else if (i%2 != 0 && getTile(r, c).getChip() != Chip.NONE) {
//             throw new InvalidMoveException("Error: UNDO trying to jump chip over a NON-empty square (" + r + ", " + c + ")");
//           }
//         }
    ...
    if (apply) {
      /* move our piece from the old location to the new one */
      changeTile(move.pointFrom().row(), move.pointFrom().col(), friend/*Chip.NONE*/);
      changeTile(move.pointTo().row(), move.pointTo().col(), Chip.NONE/*friend*/);

      /* INSERT captured pieces */
      for (int i=1, r=row+rowChange, c=col+colChange;  // init indicies
          r!=move.pointFrom().row() || c!=move.pointFrom().col();  // go until we get to the "to" point 
          i++, r+=rowChange, c+=colChange)  // update indicies
      {
        if (i%2 != 0) {
          changeTile(r, c, enemy);
        }
      }
